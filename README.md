...
To test Cloud Controller Manager integration, you can deploy a test application:

```bash
kubectl apply -f ./nginx-demo.yaml
```

Wait for LoadBalancer service EXTERNAL-IP be created:

```bash
kubectl get svc/nginx-service --watch
```

Finally access on your browser http://{your-service-external-ip}/

After check you can destroy the test application:
...
- deploys official Oracle [Cloud Controller Manager][ccm] to support LoadBalancer services
...
To keep on free tier boudinaries only 1 load balancer can be create with shape of 10Mbps (adding annotation `service.beta.kubernetes.io/oci-load-balancer-shape: 10Mbps` on your service metadata)
